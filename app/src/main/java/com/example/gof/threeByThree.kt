...
class threeByThree  : AppCompatActivity() {
    ...
    private fun initializePuzzle() {
        // Generate a list of tiles (numbers 1 to 8 and one empty space)
        tiles = mutableListOf(1, 2, 3, 4, 5, 6, 7, 8, 9)
...
//    private fun initializePuzzle() {
//        // Manually set up a simple arrangement of tiles (1 to 8 and an empty space)
//        tiles = mutableListOf(1, 2, 3, 4, 5, 6, 7, 8, 9)  // Using 9 as the empty space
//
//        // This arrangement creates a puzzle layout that is easy to solve
//        // You can customize this arrangement to set up different test scenarios
//        // For example, you can arrange them like this for a simple solvable puzzle:
//        tiles = mutableListOf(1, 2, 3, 4, 5, 6, 7, 9, 8)  // '9' is the empty space
//
//        // You can also shuffle the tiles (excluding the last tile which is the empty space)
//        // This can be useful for testing the shuffle functionality:
//        // tiles.shuffle()
//        // Ensure the shuffled puzzle is solvable
//        // while (!isSolvable(tiles)) {
//        //     tiles.shuffle()
...
//    }
    ...
    private fun moveTile(position: Int) {
        ...
        val emptyPosition = tiles.indexOf(puzzleSize * puzzleSize)

        val completionTime3 = timerTextView.text.toString()
        ...
        if (isWinning()) {
            ...
            var initialbestCompletionTime = sharedPreferences.getString("bestCompletionTime3", "N/A")
            Log.d("MainActivity", "intialllll best Time: $initialbestCompletionTime")

            //intially add the first score as the best value
            if(initialbestCompletionTime=="N/A"){
                initialbestCompletionTime=completionTime3
                with(sharedPreferences.edit()) {
                    putString("finalBestTime3", initialbestCompletionTime)
                    apply()
                }
                with(sharedPreferences.edit()) {
                    putString("bestCompletionTime3", initialbestCompletionTime)
                    apply()
                }
            }
            ...
            val finalNewBestTime = sharedPreferences.getString("finalBestTime3", "N/A") ?: "N/A"
            Log.d("MainActivity", "Old Completion Time: $oldCompletionTime")
            Log.d("MainActivity", "final best Completion Time: $finalNewBestTime")



            with(sharedPreferences.edit()) {
                putString("completionTime3", timerTextView.text.toString())
                putInt("puzzleSize", puzzleSize)
                apply()
            }


            if (compareTimes(completionTime3, finalNewBestTime)) {
                val newBestTime = timerTextView.text.toString()
                with(sharedPreferences.edit()) {
                    putString("bestCompletionTime3", newBestTime)
                    apply()
                }
...
//                }
    ...
    private fun compareTimes(time1: String, time2: String): Boolean {
        ...
        return if (minutes1.toString()==minutes2.toString() && seconds1.toString() < seconds2.toString() || minutes1.toString()<minutes2.toString() ) {
            true
        } else {
            false
        }
    ...
    private fun addTilesToGrid() {
        ...
        val buttonMargin = 40 // Adjust the margin value as needed
        ...
        for (i in 0 until totalTiles) {
            ...
            if (tiles[i] != puzzleSize * puzzleSize) {
                // Set button text for tiles 1 to 8 (or non-empty tiles)
            ...
            // Calculate row and column indices for the 3x3 grid
            val row = i / puzzleSize
            val col = i % puzzleSize

            // Create GridLayout.LayoutParams for the button
            val params = GridLayout.LayoutParams().apply {
                rowSpec = GridLayout.spec(row)
                columnSpec = GridLayout.spec(col)
            }

            // Add button to the GridLayout with specified layout parameters
    ...
    private fun isWinning(): Boolean {
        val winningConfiguration = listOf(1, 2, 3, 4, 5, 6, 7, 8, 9) // Winning configuration: [1, 2, 3, ..., 8, 0]
        ...
        Log.d("MainActivity", "isWinning: $isWinning")
        return isWinning
    }
